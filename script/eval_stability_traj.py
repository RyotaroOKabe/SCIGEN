...
# pstruct_list = get_pstruct_list(num_atoms, frac_
# , atom_types, lattices, atom_type_prob=True)
# traj_pstruct_list = get_traj_pstruct_list(num_atoms, all_frac_coords, all_atom_types, all_lattices, atom_type_prob=False)
t_step = 20
adjust_r_max = True
t_min = None
traj_pstruct_list, t_list = get_traj_pstruct_list(num_atoms, all_frac_coords, all_atom_types, all_lattices, t_step=t_step, atom_type_prob=False)
traj_pstruct_list = traj_pstruct_list[:30000]
num = len(traj_pstruct_list)
survival_ratio1 = []
survival_ratio2 = []
survival_ratio3 = []
...
for i_t, t in enumerate(t_list):
    ...
    if t_min is not None:
        if t<t_min:
            print(f'[[{i_t}]] time step: {t}: skip!!')
            continue

    start_time1 = time.time()
    print('\n')
    print(f'[[{i_t}]] time step: {t}')
    pstruct_list = [traj_pstruct_list[i][i_t] for i in range(num)]
    ...
    survival_ratio1.append(len(astruct_list)/num)   # filter1: charge neutrality

    if len(astruct_list) == 0:
        print(f'[filter1] No valid structures at time step {t}')
        survival_ratio2.append(0.0)
        survival_ratio3.append(0.0)
        survival_ratio_final.append(0.0)        
        continue
    # #%%
    ...
    survival_ratio2.append(len(mpdata)/num) # filter2: space occupancy  #!!
    if len(mpdata) == 0:
        print(f'[filter2] No valid structures at time step {t}')
        survival_ratio3.append(0.0)
        survival_ratio_final.append(0.0)      
        continue
    # mpdata_file = join('./ehull_prediction/data/mpdata_mp20_test.pkl')  #!
    # mpdata = pd.read_pickle(mpdata_file)    #!
    dataset = Dataset_InputStability(mpdata, r_max, target, descriptor, scaler, nearest_neighbor, adjust_r_max)  # dataset
    num1 = len(dataset)
    idx_te = range(num1)
    te_set = torch.utils.data.Subset(dataset, idx_te)
    run_time1 = time.time() - start_time1
    total_num1 = len(mpdata)
    print(f'Total outputs:{total_num1} materials')
    print(f'run time: {run_time1} sec = {convert_seconds_short(run_time1)}')
    print(f'{run_time1/total_num1} sec/material')
    ...
    survival_ratio3.append(len(id_stable)/num) # filter3: Ehull classification with GNN #!!
    if len(id_stable) == 0:
        print(f'[filter3] No valid structures at time step {t}')
        survival_ratio_final.append(0.0)      
        continue
    ...
    print(f'{run_time2/total_num1} sec/material')
    ...
    dataset2 = Dataset_InputStability(mpdata2, r_max, target, descriptor, scaler, nearest_neighbor, adjust_r_max)
    num2 = len(dataset2)
    idx_te2 = range(num2)
    ...
    run_time2 = time.time() - start_time2
    survival_ratio_final.append(len(id_stable2)/num)
...
print('t_list: ', t_list)
print('survival_ratio1: ', survival_ratio1)
print('survival_ratio2: ', survival_ratio2)
print('survival_ratio3: ', survival_ratio3)
print('survival_ratio_final: ', survival_ratio_final)

plt.plot(t_list, survival_ratio_final, label='Final')
plt.plot(t_list, survival_ratio1, label='Filter1: Charge neutrality')
plt.plot(t_list, survival_ratio2, label='Filter2: Space occupancy')
plt.plot(t_list, survival_ratio3, label='Filter3: Ehull classification with GNN')
plt.legend()
plt.savefig(f'survival_count_{out_name}_{t_step}.png')
